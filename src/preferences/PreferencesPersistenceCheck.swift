...
enum PreferencesPersistenceCheck {
    ...
    /// explicit suites (usage stats). System suites we only read (e.g.
    ...
    private static func showDialog(symlinkedPaths: [String], unwritablePaths: [String]) {
        ...
        alert.messageText = NSLocalizedString("Your settings reset every time CmdTab restarts.", comment: "")
    ...
    private static func dialogContent(symlinkedPaths: [String], unwritablePaths: [String]) -> (message: String, paths: [String]) {
        if !symlinkedPaths.isEmpty {
            return (NSLocalizedString("CmdTab preferences file is a symlink. macOS doesn’t support symlink for preferences. Please use a valid file instead of a symlink here:", comment: ""), symlinkedPaths)
        }
        return (NSLocalizedString("CmdTab preferences file isn’t writable by your account. Please fix ownership and permissions of the file here:", comment: ""), unwritablePaths)
    }
