...
class ShowHideIllustratedView {
    ...
    static let hideStatusIconsSubtitle = NSLocalizedString("CmdTab will show if the window is currently minimized or fullscreen with a status icon.", comment: "")
...
class AppearanceTab: NSObject {
    ...
    private static func makeAppearanceView() -> NSView {
        ...
        let styleButtons = LabelAndControl.makeImageRadioButtons("appearanceStyle", AppearanceStylePreference.visibleCases,
            rawValues: AppearanceStylePreference.visibleCases.map(\.indexAsString), extraAction: { _ in
