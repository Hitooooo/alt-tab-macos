...
class FeedbackWindow: NSWindow {
    ...
    /// successful handoff to GitHub. Called from init and from `App.showFeedbackPanel` on each reopen.
    func reset() {
        captureCurrentDraft()
        kind = .bug
        showKindPicker()
    ...
    @objc private func sendCallback() {
        ...
        alert.messageText = NSLocalizedString("Your browser will open a prefilled public GitHub issue.", comment: "")
        alert.informativeText = NSLocalizedString("Review the issue on GitHub before submitting it. A debug profile is included to help diagnose the issue.", comment: "")
        alert.addButton(withTitle: NSLocalizedString("Open GitHub", comment: ""))
        let cancelButton = alert.addButton(withTitle: NSLocalizedString("Cancel", comment: ""))
        cancelButton.keyEquivalent = "\u{1b}" // Escape
        if alert.runModal() != .alertFirstButtonReturn { return }
        guard let url = makeGitHubIssueUrl(), NSWorkspace.shared.open(url) else { return }
        drafts[kind] = nil
    ...
    private func makeGitHubIssueUrl() -> URL? {
        var components = URLComponents(string: App.repository + "/issues/new")
