...
class TileView: FlippedView {
    ...
    private func updateLabelTooltipIfNeeded() {
        guard Appearance.style != .appIcons else { return }
    ...
    private func updateValues(_ element: Window, _ index: Int, _ newHeight: CGFloat) {
        ...
        let hideStatusIcons = Preferences.hideStatusIcons
        statusIcons.update(
            isHidden: element.isHidden && !hideStatusIcons,
            isFullscreen: element.isFullscreen && !hideStatusIcons,
            isMinimized: element.isMinimized && !hideStatusIcons,
    ...
    private func updatePositions(_ newHeight: CGFloat) {
        ...
        if Appearance.style == .thumbnails {
            let hHeight = max(appIcon.frame.height, TilesView.layoutCache.labelHeight)
    ...
    private func updateDockLabelIconPosition() {
        ...
        let offset = (iconSize * (Appearance.style == .appIcons && Appearance.resolvedSize == .large ? 0.03 : 0.05)).rounded()
    ...
    private func windowlessIndicatorYPosition() -> CGFloat {
        let verticalOffset = Appearance.style == .titles ? CGFloat(5) : CGFloat(10)
        if Appearance.style == .thumbnails {
    ...
    private func setFrameWidthHeight(_ newHeight: CGFloat) {
        ...
        assignIfDifferent(&frame.size.width, Self.layoutWidth(window_))
        assignIfDifferent(&frame.size.height, newHeight)
    }

    static func layoutWidth(_ element: Window) -> CGFloat {
        let style = Appearance.style
        let contentWidth: CGFloat
        if style == .thumbnails {
            let hasThumbnail = element.thumbnail != nil
            let imageSize = hasThumbnail ? element.size : element.icon?.size()
            contentWidth = thumbnailSize(imageSize, !hasThumbnail).width
        } else if style == .titles {
            contentWidth = maxThumbnailWidth() - Appearance.edgeInsetsSize * 2
        } else {
            contentWidth = Appearance.iconSize
        }
        let frameWidth = (contentWidth + Appearance.edgeInsetsSize * 2).rounded()
        return max(frameWidth, minThumbnailWidth()).rounded()
    }
